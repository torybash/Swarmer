...
public static class BoidTools
{
  ...
  {
    public SeparationForce( Monster.Settings sts )
    {
      ...
      //solve( {optFactor / OptDistance = OptDistance / 2}, {optFactor} );
			optFactor = sts.OptDistance * sts.OptDistance / 2f;
//			optFactor = sts.OptDistance *;
		}

    public bool Calc( Vector2 cur, Vector2 other, out Vector2 force )
    {
  ...
  {
    public CollisionAvoidanceForce( Monster.Settings sts, float sepForceAtOptDistance )
    {
      ...
      // plot( eval(f(x), eval(Res, RealConsts) ), x = 0..eval(ViewRadius, RealConsts) );
        ...
        factor2 = -0.5f * sts.SpeedMultipliyer * sepForceAtOptDistance * sts.OptDistance / ( sts.OptDistance - sts.ViewRadius );
      #endif
    }
    ...
    public bool Calc( Vector2 cur, Vector2 birdDir, Collider2D cld, out Force force )
    {
      ...
      if( dist <= MathTools.epsilon )
      {
		...
		Vector2 cldPos = cld.transform.position;
		revDir = (pointOnBounds - cldPos).normalized;
